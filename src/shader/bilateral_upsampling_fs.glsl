...
uniform sampler2D s0;   // color low-resolution.
uniform sampler2D s1;	// normal/depth hi-resolution
uniform sampler2D s2;	// normal/depth low-resolution
...
void main()
{
	...
	switch (idx) {
	case 0:
		lowResNmlDepth[0] = fetch(s2, uv, vec2(0, 0));
		lowResNmlDepth[1] = fetch(s2, uv, vec2(1, 0));
		lowResNmlDepth[2] = fetch(s2, uv, vec2(0, 1));
		lowResNmlDepth[3] = fetch(s2, uv, vec2(1, 1));
		...
		break;
	case 1:
		lowResNmlDepth[0] = fetch(s2, uv, vec2(-1, 0));
		lowResNmlDepth[1] = fetch(s2, uv, vec2(0, 0));
		lowResNmlDepth[2] = fetch(s2, uv, vec2(-1, 1));
		lowResNmlDepth[3] = fetch(s2, uv, vec2(0, 1));
		...
		break;
	case 2:
		lowResNmlDepth[0] = fetch(s2, uv, vec2(0, -1));
		lowResNmlDepth[1] = fetch(s2, uv, vec2(1, -1));
		lowResNmlDepth[2] = fetch(s2, uv, vec2(0, 0));
		lowResNmlDepth[3] = fetch(s2, uv, vec2(1, 0));
		...
		break;
	case 3:
		lowResNmlDepth[0] = fetch(s2, uv, vec2(-1, -1));
		lowResNmlDepth[1] = fetch(s2, uv, vec2(0, -1));
		lowResNmlDepth[2] = fetch(s2, uv, vec2(-1, 0));
		lowResNmlDepth[3] = fetch(s2, uv, vec2(0, 0));
	...
	vec4 hiResNmlDepth = texture2D(s1, uv);
